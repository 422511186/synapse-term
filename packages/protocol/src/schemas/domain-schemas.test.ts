...
describe('domain schemas', () => {
  ...
  it('parses a session state and rejects unknown fields', () => {
    ...
    expect(
      sessionStateSchema.parse({ ...session, sharedAt: '2026-08-02T00:00:00.000Z' }),
    ).toMatchObject({ sharedAt: '2026-08-02T00:00:00.000Z' });
  });
  ...
  it('parses conversation, turn, structured model item, and tool call records', () => {
    ...
    expect(agentConversationSchema.parse(conversation)).toEqual({
      ...conversation,
      driver: 'builtin',
    });
  ...
  it('defaults legacy conversations and turns to the builtin driver', () => {
    ...
    expect(agentConversationSchema.parse(legacyConversation)).toMatchObject({
      driver: 'builtin',
    });
    ...
    expect(agentTurnSchema.parse(legacyTurn)).toMatchObject({ driver: 'builtin' });
  });

  it('parses external driver turns without a model selection and rejects builtin turns without one', () => {
    const externalTurn = {
      id: 'turn-acp',
    ...
    expect(agentTurnSchema.parse(externalTurn)).toEqual(externalTurn);
    expect(agentTurnSchema.safeParse({ ...externalTurn, driver: 'builtin' }).success).toBe(false);
  });

  it('parses external agent tasks without a provider profile', () => {
    const externalTask = {
      id: 'task-acp',
      sessionId: 'session-1',
      goal: 'Check disk usage',
    ...
    expect(agentTaskSchema.parse(externalTask)).toEqual(externalTask);
  });
