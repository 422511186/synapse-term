...
const leaseOwnerSchema = z.discriminatedUnion('kind', [
  ...
  z.strictObject({ kind: z.literal('external'), callerId: idSchema }),
...
export const agentConversationSchema = z.strictObject({
  id: idSchema,
  sessionId: idSchema,
  driver: agentDriverKindSchema.default('builtin'),
...
export const agentTurnSchema = z
  ...
  .superRefine((turn, context) => {
    if (turn.driver === 'builtin' && turn.model === undefined) {
      context.addIssue({
        code: 'custom',
        path: ['model'],
        message: 'builtin turn requires a model selection',
      });
    }
  });
...
export const agentTaskSchema = z.strictObject({
  id: idSchema,
  sessionId: idSchema,
  providerProfileId: idSchema.optional(),
  goal: z.string().min(1),
  status: z.enum([
    'queued',
    'running',
...
export const externalCallerSchema = z.strictObject({
  kind: z.enum(['mcp', 'acp']),
  id: idSchema,
