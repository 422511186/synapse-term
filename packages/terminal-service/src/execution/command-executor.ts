...
import type { PtyDisposable } from '../shell/pty-adapter.js';
import type { SessionActor } from '../session/session-actor.js';
import { type SessionActorEvent } from '../session/session-actor.js';
import {
  resolveShellDriver,
...
export interface ExecuteCommandInput {
  ...
  taskId: string;
  /** 租约持有方：默认内置 Agent；external 表示外部调用者（MCP / ACP，ADR-0024） */
  ...
  execute(input: ExecuteCommandInput): Promise<CommandExecutionResult> {
    ...
    if (ownerKind === 'external') {
      if (
        ...
        return Promise.reject(
          new CommandExecutorError('lease_not_owned', 'external caller does not own the lease'),
        );
      }
    } else if (
