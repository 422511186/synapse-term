...
describe('CommandExecutor', () => {
  ...
  it('executes a command as an external caller holding an external lease', async () => {
    const pty = new FakePty(125);
    const actor = new SessionActor('session-external', pty, { executionDialect: 'posix' });
    await actor.markPtyRunning();
    await actor.transitionShell('probing');
    await actor.transitionShell('ready');
    await actor.verifyCurrentEnvironment('posix', 'unix');
    ...
    const executor = new CommandExecutor(actor, {
      nonceFactory: () => 'txn-external',
      observationWindowMs: 5_000,
    });
    const execution = executor.execute({
      transactionId: 'transaction-external',
      taskId: 'mcp-client',
      ownerKind: 'external',
      leaseEpoch: lease.value.lease.epoch,
      command: 'ls -la',
    ...
    await waitForDispatch(actor, pty);
    await startCapture(actor, pty);
    pty.emitData('total 0\r\n$ \u001b]777;TA;txn-external;0\u0007');

    await expect(execution).resolves.toMatchObject({
      status: 'completed',
      transaction: { status: 'completed', exitCode: 0, taskId: 'mcp-client' },
    });
    expect(actor.snapshot.lease).toEqual({
      owner: { kind: 'external', callerId: 'mcp-client' },
      epoch: 1,
    });
    actor.dispose();
  });

  it('rejects an external caller that does not own the current lease', async () => {
    const pty = new FakePty(126);
    const actor = new SessionActor('session-external-reject', pty, {
      executionDialect: 'posix',
    });
    await actor.markPtyRunning();
    await actor.transitionShell('probing');
    ...
    const executor = new CommandExecutor(actor, {
      nonceFactory: () => 'txn-external-reject',
      observationWindowMs: 5_000,
    });
    await expect(
      executor.execute({
        transactionId: 'transaction-external-reject',
        taskId: 'mcp-client',
        ownerKind: 'external',
        leaseEpoch: lease.value.lease.epoch,
        command: 'ls -la',
