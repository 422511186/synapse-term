...
import { type SessionActor, type AgentWriteInput } from '../session/session-actor.js';
import {
  resolveShellDriver,
  shellInputLines,
  type ShellDriver,
...
export interface PlaintextDispatchInput {
  ...
  /** 租约持有方：默认内置 Agent；external 表示外部调用者（ADR-0024） */
  ...
  prepare(input: PlaintextDispatchInput): DispatchResult {
    ...
    if (ownerKind === 'external') {
      if (
        ...
        return this.#protocolError(
          'External caller lease is no longer valid for plaintext dispatch',
        );
      }
    } else if (
      snapshot.lease.epoch !== input.leaseEpoch ||
...
async function writeToPty(
  actor: SessionActor,
  writeInput: Pick<AgentWriteInput, 'taskId' | 'ownerKind' | 'leaseEpoch'>,
  payload: string,
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const result = await actor.writeAgent({
      ...
      ...(writeInput.ownerKind === undefined ? {} : { ownerKind: writeInput.ownerKind }),
