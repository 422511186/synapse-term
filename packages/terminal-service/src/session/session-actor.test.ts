...
describe('SessionActor', () => {
  ...
  it('marks the Session as externally addressable only after the user copies the id', async () => {
    const pty = new FakePty(123);
    const actor = new SessionActor('session-1', pty);
    expect(actor.snapshot.sharedAt).toBeUndefined();

    await actor.markShared();
    expect(actor.snapshot.sharedAt).toBeDefined();
    expect(actor.snapshot.lease).toEqual({ owner: { kind: 'user' }, epoch: 0 });
    expect(actor.snapshot.attachment).toBe('detached');
  });
});
