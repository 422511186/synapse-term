...
describe('mcp-runtime dependency direction', () => {
  it('does not depend on Desktop or Session runtime implementation', async () => {
    const root = fileURLToPath(new URL('.', import.meta.url));
    const files = await collectImplementationFiles(root);
    for (const file of files) {
      const source = await readFile(file, 'utf8');
      expect(source, file).not.toMatch(/apps[\\/]desktop/);
      expect(source, file).not.toMatch(/@synapse-term\/session-runtime/);
