...
export function auditTypeZh(type: string): string {
  const labels: Record<string, string> = {
    ...
    'model.created': '创建模型配置',
    'model.updated': '更新模型配置',
    'model.tested': '检测模型配置',
    'model.enabled': '启用模型配置',
    'model.disabled': '停用模型配置',
    'model.default_changed': '切换默认模型',
    'model.removed': '删除模型配置',
    'external.command': '执行外部命令',
    'external.wait': '等待外部命令',
    'external.interrupt': '中断外部命令',
    'external.denied': '拒绝外部命令',
    'external.rejected': '拒绝外部调用',
    'external.observe': '观察外部终端',
    'approval.requested': '请求批准命令',
    'approval.granted': '批准命令',
    'approval.rejected': '拒绝命令',
  };
