...
const keyringFactory: CredentialEntryFactory = {
  create(service, account) {
    // keyring 原生模块按需加载：桌面主进程不持有 SecretStore 时无需解析
    let entryPromise: Promise<AsyncEntry> | undefined;
    const loadEntry = (): Promise<AsyncEntry> => {
      entryPromise ??= import('@napi-rs/keyring').then(
        ({ AsyncEntry: KeyringAsyncEntry }) => new KeyringAsyncEntry(service, account),
      );
      return entryPromise;
    };
    return {
      setPassword: async (password) => (await loadEntry()).setPassword(password),
      getPassword: async () => (await loadEntry()).getPassword(),
      deletePassword: async () => (await loadEntry()).deleteCredential(),
    };
  },
};
