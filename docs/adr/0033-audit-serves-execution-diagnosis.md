...
- Successful read-only Audit Observations are excluded from the default investigation list but remain retained and discoverable through an explicit observation filter; failed observations remain visible by default.
- While the audit workspace is open, the current query refreshes periodically without discarding filters or the selected detail; leaving the workspace stops refreshes, and the first version may use bounded polling because no audit push channel exists.
- Audit Search covers only redacted operation/path summaries, reasons, Session identifiers, and correlation IDs; it does not search or expose complete terminal output or secrets.
...
- The first version stays in-app and read-only without an audit export; any future export must be a separately designed redacted report.
