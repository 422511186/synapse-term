...
import { createProviderProfile } from '@terminal-agent/domain';
...
describe('ProviderModelDiscoveryService', () => {
  it('is a public Core service', () => {
    expect(core).toMatchObject({ ProviderModelDiscoveryService: expect.any(Function) });
  });

  it('deduplicates, sorts, and bounds discovered models', async () => {
    const Service = (
      core as typeof core & {
        ProviderModelDiscoveryService: new (options: Record<string, unknown>) => {
          discover(
  ...
  it('maps cancellation and total timeout to stable discovery errors', async () => {
    ...
    const Service = core.ProviderModelDiscoveryService;
