...
import type { AgentTaskScheduler } from './agent-task-scheduler.js';
import { ApprovalManager, hashCommand } from './approval-manager.js';
import type { AuditService } from './audit-service.js';
import { CommandExecutor, type CommandExecutorEvent } from './command-executor.js';
...
import type { CoreSecretStore, ProviderAdapterFactory } from './core-request-router.js';
import { ShellProbe } from './shell-probe.js';
import type { SessionActor } from './session-actor.js';
import type { SessionManager } from './session-manager.js';
import { TerminalToolGateway, type ToolGatewayResult } from './tool-gateway.js';
import type { PolicyEngine } from './policy-engine.js';
import type { ModelAdapter, ModelEvent, ModelInputItem } from './model-adapter.js';
import type { LocalFileService } from './local-file-service.js';
import type { LocalFilePolicy } from './local-file-policy.js';
...
export class AgentCoordinator {
  ...
  async start(
    ...
    const queuedTurn = createAgentTurn({
      ...
      conversationId: conversation.id,
      sessionId,
      ...createAgentModelSelection(profile, model),
      reasoningEffort,
  ...
  async history(sessionId: string): Promise<AgentHistoryView> {
    ...
    return {
      ...
      turns: this.#repositories.listAgentTurns(conversation.id).map((turn) => ({
        ...
        supportedReasoningEfforts: [...turn.supportedReasoningEfforts],
      })),
      items: this.#repositories.listModelItems(conversation.id),
      ...(state === undefined ? {} : { activeTurnId: state.turn.id }),
    };
  }
...
class ApprovalAwareGateway implements RuntimeToolGateway {
  ...
  async callWithContext(
    ...
    this.#stateCoordinatorMarkResuming(toolCallId);
    const result = await this.#state.gateway.call(name, argumentsValue, this.#grant, {
      toolCallId,
    });
    this.#stateCoordinatorRecordResult(toolCallId, result);
  ...
  #stateCoordinatorMarkResuming(toolCallId: string): void {
    this.#onToolStart(toolCallId);
  }

  #stateCoordinatorRecordResult(toolCallId: string, result: ToolGatewayResult): void {
...
function extractTransactionId(result: ToolGatewayResult): string | undefined {
...
function isWaitingApproval(value: unknown): value is {
  decision: unknown;
...
function nextModelSequence(items: readonly ModelItem[]): number {
  return items.reduce((next, item) => Math.max(next, item.sequence + 1), 0);
}

function commandTimelineStatus(status: string, exitCode: number | undefined): string {
  return status === 'completed' && exitCode !== undefined && exitCode !== 0 ? 'failed' : status;
}

function buildSessionSummary(snapshot: SessionState): string {
  return [
