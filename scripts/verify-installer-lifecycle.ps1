...
if ([string]::IsNullOrWhiteSpace($SetupPath)) {
  $SetupPath = Join-Path $workspace 'release\Terminal-Agent-0.2.0-x64-Setup.exe'
}
...
try {
  ...
  [IO.File]::WriteAllText(
    $statePath,
    "[core]`r`nrunning=1`r`npid=$PID`r`nversion=0.2.0`r`nsessions=2`r`nagentTasks=1`r`nupdatedAt=$([DateTime]::UtcNow.ToString('o'))`r`n",
    [Text.Encoding]::ASCII
  )
