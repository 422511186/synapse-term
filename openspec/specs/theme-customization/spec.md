...
桌面端 MUST 允许用户自定义核心配色与终端文字配色：核心配色包括背景（background）、前景（foreground）与强调色（accent），终端文字配色包括终端前景与 16 个 ANSI 颜色（black/red/green/yellow/blue/magenta/cyan/white 及其 bright 变体）；自定义配色 MUST 有显式启用开关，关闭时 MUST 使用内置浅色/深色主题的默认值。核心配色 MUST 提供一键重置为当前有效 scheme 内置值的操作。颜色输入 MUST 为合法的六位十六进制颜色，非法值 MUST 在持久化时被拒绝并保留原值。
...
#### Scenario: Preview colors before returning to the terminal
- **WHEN** 用户在外观设置修改主题模式或自定义配色
- **THEN** 页面内的终端配色预览 MUST 使用与实际终端相同的主题解析结果更新，不创建 PTY 或 Session

#### Scenario: Reveal advanced terminal colors
- **WHEN** 用户首次打开外观设置
- **THEN** 主题模式和核心配色 MUST 优先显示，16 色终端文字编辑器 MUST 默认折叠；用户可以展开编辑，折叠状态不得清除已经保存的颜色
...
#### Scenario: Reset custom core colors
- **WHEN** 用户启用自定义配色并点击“重置核心配色”
- **THEN** 背景、前景与强调色 MUST 重置为当前有效 scheme 的内置核心配色
- **AND** 自定义配色开关 MUST 保持启用，终端文字配色 MUST 保持不变
