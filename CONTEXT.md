...
需要持续读取交互输入或不会返回当前 Shell 提示符的命令；明确识别为此类的命令不适合 `synapse_execute` 的结构化外部执行，以 `INTERACTIVE_COMMAND_UNSUPPORTED` 拒绝，由外部客户端改用外部输入启动并驱动。无法静态判定的命令不因此被视为交互式命令，而按风险分类和完成证据结果处理。
_Avoid_: 远程会话、后台任务

**外部输入（External Input）**:
外部客户端通过 `synapse_input` 向已共享 Session 的 PTY 写入的交互输入，由可打印文本与白名单特殊键组成，一次调用按先文本后按键的顺序写入；它不能携带任意转义序列或原始字节，写入后在本地终端 UI 自然回显。
_Avoid_: 远程按键、键盘事件

**事务内输入（Transactional Input）**:
挂靠某个进行中外部事务的外部输入，用于帮助该事务收敛（如 sudo 密码）；它继承原事务的审批结果，不校验也不轮换执行上下文 ID，不递增能力代际，前提只有事务存在且未达终态。
_Avoid_: 事务续期、命令追加

**自由输入（Free Input）**:
不挂靠任何外部事务的外部输入，用于驱动交互式程序或键盘导航（如 vim、堡垒机菜单）；它必须携带有效的执行上下文 ID，写入后轮换该标记并保守递增能力代际，后续外部命令须先重新观察。它不是免审批通道。
_Avoid_: 旁路执行、免审批通道
