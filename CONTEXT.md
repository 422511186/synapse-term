...
**Audit Record（审计记录）**:
对 Agent 或用户操作的不可变结构化记录，包括谁发起的、针对哪个会话、适用了什么授权、观察到了什么结果，用于安全与执行诊断；它不意味着保留完整终端字节流。
_Avoid_: 终端录制、转录

**Audit Investigation（审计排查）**:
用户通过审计记录还原一次操作的发起者、目标、策略与审批决定、执行结果及失败原因的诊断活动；它独立于 Agent 对话历史和终端录像。
_Avoid_: 聊天记录、终端回放

**Audit Trace（审计事件链）**:
围绕一次 Agent Task 或外部调用按时间排列的相关审计事件集合，用于从整体结果深入到策略、审批和执行过程；没有任务关联的系统或配置操作可以作为独立事件存在。
_Avoid_: 原始日志流、对话时间线

**Redacted Audit Detail（脱敏审计详情）**:
为帮助用户识别操作内容而保留的命令或路径可读摘要，已经移除秘密和受保护输入；它不包含完整终端输出或终端录像。
_Avoid_: 原始命令记录、终端转录

**Audit Scope（审计范围）**:
一次审计查询所覆盖的 Session、Task、发起者或时间区间；全局审计默认覆盖所有保留记录，具体 Session 只是可选的缩小范围。
_Avoid_: 当前会话限定、日志文件范围

**Audit Query Window（审计查询窗口）**:
审计排查时主动选择的时间范围，默认从最近记录开始，并可扩展到当前保留期内的历史；它只限制查看范围，不改变审计记录的留存策略。
_Avoid_: 日志清理期限、会话生命周期

**Audit Filter（审计筛选）**:
用于缩小审计范围的用户可理解条件，包括时间、Session、发起者、操作类别、最终结果和风险等级；内部 Task ID 是详情关联信息，不是日常主筛选。
_Avoid_: 数据库字段筛选、原始事件查询

**Audit Detail Panel（审计详情面板）**:
在保留审计列表和筛选上下文的同时，展示一条 Audit Trace 的关联主体、脱敏详情及按时间排列的事件链的只读区域。
_Avoid_: 审计编辑页、原始载荷查看器

**Audit Retention Policy（审计保留策略）**:
规定审计记录和原始终端日志可被查询的最长期限；过期数据可以被受控清理，但审计记录不是可由用户随意清空的对话历史。
_Avoid_: 清空审计、聊天记录保留

**Audit Outcome（审计结果）**:
对一条 Audit Trace 的终态进行统一归类，包括进行中、成功、失败、拒绝、中断或信息；它是面向排查的结果，而不是某个底层事件类型的直接翻译。
_Avoid_: 原始状态字符串、事件类型

**Audit Observation（审计观察）**:
对 Session、外部调用或资源状态的只读观察记录；成功观察默认不进入审计排查主列表，观察失败仍属于需要关注的诊断事件，所有记录仍可查询。
_Avoid_: 终端录像、无关日志

**Audit Search（审计搜索）**:
在当前 Audit Scope 内按脱敏操作摘要、原因、Session 标识或关联 ID 查找审计记录的能力；搜索范围不包含完整终端输出或秘密。
_Avoid_: 全文终端搜索、原始载荷搜索
...
**Settings Workspace（设置工作区）**:
用于管理应用级配置和查看审计信息的独立工作区域，与当前 Terminal Session 工作区分离；它包含一组设置主题及其对应内容。
_Avoid_: 设置弹窗、全局下拉菜单

**Settings Topic（设置主题）**:
设置工作区中一个可独立进入和操作的配置或诊断区域，例如服务商配置、模型配置、MCP 服务、ACP 集成或审计日志；导航分组标题本身不是设置主题。
_Avoid_: 配置大杂烩、设置分组页
