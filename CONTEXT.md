...
**更新候选（Update Candidate）**:
适配当前应用平台、版本高于当前运行版本的一个固定正式发布；下载和安装确认都绑定同一个候选，不能在确认后切换发布。
_Avoid_: 最新安装链接、任意更新源

**安装确认（Installation Confirmation）**:
用户针对一个已校验更新候选与当前活动 Session 集合授予的一次性本机安装授权；安装将结束 Session，普通退出和已下载状态不构成授权。
_Avoid_: MCP 审批、退出自动更新、会话恢复

**更新缓存（Update Cache）**:
为校验或安装更新而保留的有限安装包数据；不包含 Session、终端输出、运行凭据或可重放的安装授权。
_Avoid_: 会话备份、历史恢复
